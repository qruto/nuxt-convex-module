...
dedicated route. By default the provider consumes the token on **any** URL with no state/origin
binding, so sign-in completes on whatever page receives it. The upstream protocol deliberately isn't
