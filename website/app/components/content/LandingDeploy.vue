...
<template>
  <div>
    ...
    <ol class="concave-2 rounded-plate m-0 grid list-none grid-cols-[repeat(auto-fit,minmax(270px,1fr))] gap-4.5 p-4.5">
      <li
        v-for="(step, index) in steps"
        :key="step.id"
        class="convex rounded-card px-5 pt-4.5 pb-5"
      >
        ...
        <div class="mb-3 [&>div]:my-0 [&_pre]:rounded-well [&_pre]:py-2.5 [&_pre]:pr-10 [&_pre]:pl-2.5 [&_pre]:text-[0.85rem] [&_pre]:leading-normal [&_pre]:wrap-break-word [&_pre]:whitespace-pre-wrap lg:[&_pre]:text-[0.76rem]">
