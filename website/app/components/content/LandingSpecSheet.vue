...
//
// Every stage carries its cut FIG. NN caption — the cards are the sheet's
// numbered figures. The well floors stay PLAIN. Each used to carry a static
// engraving in the card's band restating its mechanism (registration marks
// for parity, broadcast rings for live, ruled lines for the served
// document, graph paper, hatching, a tape, a ruler, breadboard sockets);
// under animated illustrations those patterns competed with the drawing
// rather than framing it, so they came off (2026-09-06).
interface SpecEntry {
  label: string
  stamp: string
  title: string
  ...
  <ul class="m-0 grid list-none grid-cols-1 gap-6 p-0 sm:grid-cols-2 lg:grid-cols-3">
    ...
    >
      ...
      <NuxtLink
        ...
        class="convex-0 hover:convex-2 rounded-card spec-card group flex h-full flex-col gap-1.5 px-5 pt-4 pb-6.5 no-underline transition-shadow duration-180 ease-out focus-visible:outline-2 focus-visible:outline-(--band)"
      >
        ...
        <!-- The card's working illustration on its own recessed stage; hover
             floods the stage with a whisper of the card's band. The stage is a
        ...
        <span
          class="concave-2 rounded-well spec-stage mt-1 mb-4 grid min-h-36 place-items-center overflow-hidden px-3.5 py-4"
          aria-hidden="true"
        >
