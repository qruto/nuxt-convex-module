...
//
// `withOptimisticUpdate` is the reason this file exists. Its argument type
// carries an intersection whose second half collapses to a STRING LITERAL when
// the handler returns a promise — upstream's trick for turning "optimistic
// update handlers must be synchronous" into a compile error rather than a
// silent runtime bug (the update would be rolled back before it resolved).
// Nothing exercised it, so a refactor could have dropped the branded branch and
// left every runtime test passing.
